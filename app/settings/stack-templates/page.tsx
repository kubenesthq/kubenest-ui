...
import { Layers, Database, Globe, MessageSquare, Zap, Search, Trash2, Container } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { useAuth } from '@/hooks/useAuth';
import { getDemoStackTemplates, deleteDemoStackTemplate, type DemoStackTemplate } from '@/lib/demo-store';
...
export default function StacksPage() {
  ...
  const [userTemplates, setUserTemplates] = useState<DemoStackTemplate[]>([]);

  useEffect(() => {
    setUserTemplates(getDemoStackTemplates());
  }, []);
  ...
  return (
      ...
      <div className="px-8 py-8 space-y-6 max-w-5xl">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredUser.map((template) => (
                <Card key={template.id} className="border-zinc-200 hover:border-zinc-300 hover:shadow-sm transition-all cursor-pointer group border-2 border-dashed">
                  <CardContent className="pt-5 pb-4">
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      <Badge variant="secondary" className="text-xs font-normal bg-zinc-100 text-zinc-600">
                        ...
                        {template.workload_config.name}
                      </Badge>
                      {template.addons.map((a) => (
                        <Badge key={a.addon_type} variant="secondary" className="text-xs font-normal bg-zinc-100 text-zinc-600">
        ...
        <div className="space-y-3">
          {filteredUser.length > 0 && (
            <h2 className="text-sm font-medium text-zinc-500 uppercase tracking-wide">Built-in Templates</h2>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
