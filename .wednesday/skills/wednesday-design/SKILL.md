...
## KubeNest Light Mode Override

This project uses **light mode by default**. All components must be adapted accordingly:

- Background: white / zinc-50 / zinc-100 (NOT dark backgrounds)
- Text: zinc-900 for headings, zinc-600 for body
- Cards: white with zinc-100 border or subtle shadow
- Primary: blue-600 (matches existing `--primary: 221.2 83.2% 53.3%`)
- When importing components from Aceternity/Magic UI that have dark backgrounds hardcoded, override with light equivalents
- Micro-interactions and animations are color-agnostic — keep them as-is
- Glassmorphism effects: use `bg-white/80 backdrop-blur` instead of dark variants
- Gradients: prefer subtle blue→indigo or zinc→white over neon dark gradients

Priority screens (in order): dashboard → clusters → projects → workloads → login
